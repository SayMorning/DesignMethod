...
到此，该BTREE树就已经构建完成了， BTREE树 和 二叉树 相比， 查询数据的效率更高， 因为对于相同的数据量来说，BTREE的层级结构比二叉树小，因此搜索速度快，IO压力也就更小。

更少的IO次数扫描出更多的数据，这些数据还是排好序的，处理起来也快
...
【在磁盘中二分查找效率比顺序查找低，在内存中二分查找效率比顺序查找高很多，所以mysql索引用B+树结构，treemap用红黑树（自平衡二叉树）。mysql数据在磁盘，treemap作用在内存。】
...
- 单值索引/组合索引的选择上，在高并发情况下倾向于选择组合索引。
