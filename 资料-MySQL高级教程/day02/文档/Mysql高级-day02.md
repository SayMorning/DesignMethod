...
​	存储引擎就是存储数据，建立索引，更新查询数据等等技术的实现方式 。存储引擎是基于表的，而不是基于库的，可以在创建表单额时候指定所用的存储引擎。所以存储引擎也可被称为表类型。
...
```sql
show status like 'Com_______'; 	#7个空格
```
...
- 慢查询日志 : 通过慢查询日志定位那些执行效率较低的 SQL 语句，用--log-slow-queries[=file_name]选项启动时，mysqld 写一个包含所有执行时间超过 long_query_time 秒的 SQL 语句的日志文件。在第4天的笔记中有介绍 
...
derived2：id=2，select_type=DERIVED的表
...
​		[1]定位低效率sql：①慢查询日志；②show processlist查看当前MySQL在进行的线程

​		[2]对低效率sql语句使用explain分析执行计划，了解每个字段的含义，重点关注key列信息（是否使用了索引）

​		[3]查看sql执行过程每个阶段耗时情况：①set profiling=1;启动功能 ②执行sql语句；③show profiles查看各阶段耗时情况

​		[4]trace分析做了解，功能是可以追踪执行的sql语句被优化的情况
...
如果索引了多列，要遵守最左前缀法则。指的是查询包含索引的最左前列，并且不跳过索引中的列，跟处在where后面的顺序无关。

如果where后面只有name和address筛选条件，那name的查找是用了索引的，address没有用索引
...
尽量使用覆盖索引（查询字段全都在索引中），减少select * 。

如果查询的字段都在索引中，就可以不用找到索引还回原表找出其它数据了
...
如果仅仅是尾部模糊匹配，索引不会失效。如果是头部模糊匹配，索引失效。--可使用覆盖索引解决
...
​		与9原因类似，当满足条件的数据占比太大，全表扫描可能更快，就不走索引了；占比小的时候就走索引筛选了
...
数据库会选择一个最优的索引（辨识度最高索引，匹配数据最少）来使用，并不会使用全部索引 。
...
了解了MySQL的排序方式，优化目标就清晰了：尽量使用覆盖索引减少额外的排序，通过索引直接返回有序数据。where 条件和Order by 筛选条件与查找字段顺序一致， 并且Order  by 的字段都是升序，或者都是降序。否则肯定需要额外的操作，这样就会出现FileSort。
...
对于包含OR的查询子句，如果要利用索引，则OR之间的每个条件列都必须用到索引 ， 而且不能使用到复合索引； 如果没有索引，则应该考虑增加单列索引。
