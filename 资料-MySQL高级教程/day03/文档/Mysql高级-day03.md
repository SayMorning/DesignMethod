...
1） SQL 语句不一致的情况， 要想命中查询缓存，查询的SQL语句必须一致，包括单词的大小写。
...
​										客户端A把U表上了读锁，A只能读U表，不能读其他表，也不能改A表。客户端B可以读U表和其他表，不能改U表									（阻塞状态，当A释放锁即可继续执行）

2） 写锁（排它锁）：当前操作没有完成之前，它会阻断其他写锁和读锁。

​										客户端A把U表上了写锁，A可以读U表，也可以改A表。客户端B既不可以读U表也不可以改U表
...
1.使用locak tables 给表加锁时候，必须同时给所有涉及到的表加锁，因为加锁之后，当前会话，就不能操作没有加锁的表。

2.在锁定表时候，如果加上关键字local，满足MyISAM表的并发插入问题。eg： lock table t3 read local;
...
4.myISAM锁调度问题
	MyISAM存储引擎的读锁和写锁是互斥的，读写操作室串行的，那么如果读写两个进程同时请求同一张表，Mysql将会使写进程先获得锁。不仅仅如此，即使读请求先到达锁等待队列，写锁后到达，写锁也会先执行。因为mysql认为写请求比读请求更加重要。这也正是MyISAM不适合含有大量更新操作和查询操作应用的原因。
...
| 脏读（Dirty Reads）                | 当一个事务A正在访问数据，并且对数据进行了修改，而这种修改还没有提交到数据库中，这时，另外一个事务B也访问这个数据，然后使用了这个数据。后来A放弃修改，事务回滚了。 |
| 不可重复读（Non-Repeatable Reads） | 一个事务在读取某些数据后的某个时间，再次读取以前读过的数据，却发现和以前读出的数据不一致。期间有其他事务修改过数据，并提交了。 |
...
对于UPDATE、DELETE和INSERT语句，InnoDB会自动给涉及数据集加排他锁（X)；通过commit释放锁
...
由于隔离级别是repeatable-read，在session1提交事务，session2更新数据后，session1是无法立马查到被session2更新的数据的，需要再执行一次commit才行。
...
比如条件为id<10，实际表中缺少id=3、4的数据，这两条数据就是间隙
