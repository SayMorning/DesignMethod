...
参数：
	...
	相当于备份创建库/表操作命令时，再最上面加上一句，drop database/table if exists db_name/tb_name；
...
示例 ： 
	...
	mysqldump -uroot -p2143 -T /tmp test city  --将test库的city表输出到/temp目录下
```

![1555501806693](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\1555501806693.png) 

<img src="C:\Users\ChengCai\AppData\Roaming\Typora\typora-user-images\image-20211101163656402.png" alt="image-20211101163656402" style="zoom:50%;" align="left"/>

上图这个sql文件里存放的不是直接的数据信息，而是创建ta_book时的创建语句，操作数据的语句记录。
...
```
source /root/tb_book.sql  --进入mysql里面使用
```
...
<img src="C:\Users\ChengCai\AppData\Roaming\Typora\typora-user-images\image-20211101170443875.png" alt="image-20211101170443875" style="zoom: 50%;" align="left"/>
...
```sql
#授权给192.168.192.131上的账号名为itcast密码为itcast的用户访问本机上的所有数据库
...
```sql
#告诉从机mysql以哪个用户同步哪台主机上的哪个二进制文件，明确到从哪一步开始
...
缺点：这种方法有局限性，并不通用，count值是全部数据的值，如果按条件筛选就不能拿这个值来用。
